...
unsigned long int startTimeMicros;
...
uint8_t rx1_1MAC[] = { 0x8C, 0xAA, 0xB5, 0x4F, 0xE5, 0x36 };
...
void handleStart(AsyncWebServerRequest* request) {
  ...
  // changes made due to coach requirement
  timingSequenceStarted = true;
  startTimeMicros = micros();
  Serial.println("TIMER STARTED (by /start)");
    ...
    case CMD_ACK:
      {
        ...
        // if (pkt.stage == 0 && !timingSequenceStarted) { changes made due to coach requirement
        if(pkt.stage == 0){
          timingSequenceStarted = true;
